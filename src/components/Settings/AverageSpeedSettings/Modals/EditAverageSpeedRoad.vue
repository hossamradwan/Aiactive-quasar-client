...
export default {
  ...
  methods: {
    ...mapActions('averageSpeed', ['editAverageSpeedSettings']),
    ...
    submitRoadConfiguration() {
      ...
      // copy array
      let temp = this.trapsConfiguration.slice();
      ...
      if(validateRoad && validateTraps && validatePoints) {
        ...
        this.trapsConfiguration = Array.from(temp);
        this.trapsConfiguration.oldRoadName = this.oldRoadName

        //console.log('submitting road', this.oldRoadName, this.trapsConfiguration);
        this.editAverageSpeedSettings(this.trapsConfiguration)
      }
      else {
  ...
  mounted() {
    ...
    let roadName = Object.keys(this.averageSpeedRoadData)[0];

    this.averageSpeedRoadData[roadName].forEach(
      trap => {
        this.trapsConfiguration.push(trap)
      }
    );

    this.roadName = roadName;
    this.oldRoadName = roadName;
