...
const actions = {
  getReportingData({ commit, dispatch }) {
    ...
    setTimeout(() => {
      const host = config.API_URL + "/reporting?" + 
          ...
          console.log(`success --`, response.data);
                    ...
                    if(reportedResults.hasOwnProperty('code')) {
                      if (reportedResults.code == "ER_EMPTY_QUERY")
                        showErrorMessage(`${state.activeModule.label} has no data on your System..`)
  ...
  getDistinctDates({ commit }) {
    setTimeout(() => {
      const host = config.API_URL + "/reporting_distinctDates" + 
  ...
  setSelectedData({ commit }, payload) {
    if(payload.action == 'add')
      commit('addSelectedData', payload.id);
      
    if(payload.action == 'remove')
      commit('clearSelectedData', payload.id);
  ...
  handleActionsOperation({ commit, dispatch }, payload) {
    if(payload.action == 'activeDateTime' | payload.action == 'activeModule') {
      ...
      if( !payload.firstMounted )
        dispatch('getReportingData');
...
const getters = {
  ...
  id_data: state => reportId => state.reportingData.filter(x => x.id === reportId)
};
