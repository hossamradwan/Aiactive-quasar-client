  ...
  handleStateDevices({ commit }, devices) {
    ...
    devices.forEach(device => {
      ...
      if(typeof belongTo == 'string') {
        ...
        let payload = { label: deviceName, value: deviceName, description: deviceZone, deviceModule: deviceBelongTo}
...
const getters = {
  ...
  getModuleDevices: (state) => (moduleName) => state.systemModules.filter(value => value.value == moduleName),
  ...
  getModuleActiveDevices: state => moduleName => {
    let moduleDevices = state.systemModules.filter(value => value.value == moduleName);
  ...
  getAverageSpeedActiveTraps: (state, getters, rootState, rootGetters) => {
    ...
    averageSpeedTrapsList = averageSpeedTrapsList.map(trap => `Trap ${trap.trap_number}-${trap.id}`);

    return averageSpeedTrapsList;
  },

  primitiveZonesOfPayload: state => payload => payload.map(value => value.description),

  uniqueZonesOfPayload: (state, getters) => payload => [...new Set(getters.primitiveZonesOfPayload(payload))],
